...
Манифест (tools/<каталог>/skill.json):

    {
      ...
      "doc":    "PIPELINE.md",                    исходный документ; по умолчанию
...
у всех трёх это `PIPELINE.md`, названный полем `doc` манифеста, — а сборщик
при упаковке приписывает фронтматтер из `name` и `description` манифеста
и кладёт результат как `SKILL.md`. Документ с собственным фронтматтером тоже
собирается: тогда шапка берётся из него, а имя сверяется с манифестом.
