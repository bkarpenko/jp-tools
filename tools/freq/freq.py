...
def main():
    ...
    if q == "--check":
        ...
        # Производные таблицы частотке не нужны и в её пакет не едут:
        # она читает первичные, потому что ей нужны знаменатели и полный хвост.
        sys.exit(1 if check(include_derived=False) else 0)
