...
def branch(wtype: str, in_suw: bool, in_luw2: bool,
    ...
    out.append("")
    out.append("ДАЛЬШЕ — шаг 2, такт 1: статьи СЛОВА, обе. Достать самому,")
    out.append("  у Бориса не просить: обе платформы открываются его входом.")
    out.append("  1. 日本国語大辞典 第二版 — japanknowledge.com, браузерная")
    out.append("     сессия по подписке. Это редакция по умолчанию; 精選版")
    out.append("     с kotobank — только если Борис так скомандовал.")
    out.append("  2. 漢語大詞典 — hanyucidian.org, браузерная сессия.")
    out.append("     Называть словарь, а не платформу.")
    out.append("  Статьи ЗНАКОВ — такт 2, после того как ветка выбрана:")
    out.append("  漢語大詞典 по обоим знакам. Японские параллели ветки Б")
    out.append("  тоже собирает разбор — тем же порядком (шаг Б4).")
    return out
