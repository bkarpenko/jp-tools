...
def branch(wtype: str, in_suw: bool, in_luw2: bool,
    ...
    out.append("")
    out.append("ДАЛЬШЕ — шаг 2, такт 1: статьи СЛОВА, обе стороны.")
    out.append("  Японская: 精選版日本国語大辞典 — достать самому, веб-поиск")
    out.append("     «<слово> kotobank 精選版 日本国語大辞典», потом загрузка")
    out.append("     ссылки из выдачи. У Бориса не просить. Не закрыл")
    out.append("     精選版 — запросить у Бориса 第二版.")
    out.append("  Китайская: hanyucidian.org — за подпиской, запросить")
    out.append("     у Бориса. Ответил «искал, статьи нет» — второй")
    out.append("     источник списка, ctext.org, разбор проходит сам.")
    out.append("  Сторона закрыта, только когда есть статья ЛИБО")
    out.append("  установленное отсутствие по всему её списку. Материал")
    out.append("  не пришёл и ничего о нём не сказано — это не отсутствие,")
    out.append("  а незакрытый вход: шаг 3 не начинается.")
    out.append("  Статьи ЗНАКОВ — такт 2, после того как ветка выбрана:")
    out.append("  hanyucidian по обоим знакам; ctext их не заменяет.")
    out.append("  Японские параллели ветки Б тоже собирает разбор —")
    out.append("  тем же порядком (шаг Б4).")
    return out
