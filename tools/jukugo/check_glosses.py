...
Скрипт берёт её за единственный источник правды и проверяет тело разбора:
  ...
  [E4] в слоте засвидетельствованного значения стоит ряд С
       (формулировка разбора) вместо словарной глоссы Г/Я  -> ОШИБКА
...
def main(path):
    ...
    #      (ряд Г или Я), а не формулировка разбора (ряд С): иначе вывод о знаках
    #      попадает в значение целого и формула доказывает себя сама.
    dict_en = {u["en"] for u in units if u["id"][:1] in ("Г", "Я") and u["en"]}
    SLOTS = ("**старшее значение:**", "**современное значение:**")
    for n, line in enumerate(body_lines, 1):
        t = line.strip()
        if not t.startswith(SLOTS):
            continue
        m = EN_IN_TEXT.search(t)
        if m and norm(m.group(1)) not in dict_en:
            errors.append(
                f"[E4] стр. {n}: в слоте засвидетельствованного значения стоит "
                f"не глосса словаря (Г/Я), а формулировка разбора — "
                f"\"{norm(m.group(1))[:60]}\""
            )
