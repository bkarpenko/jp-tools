...
Различение существенное. Отбрасывание колонок — свойство вопроса: рангов,
абсолютных частот и профиля по тринадцати жанрам шаг 1 не спрашивает. Отбрасывание
слов — сужение самого инструмента: слово, которого в таблице нет, он не разберёт
никогда. «Метод пока не разбирает» и «инструмент не должен знать» — разные вещи,
и второе за владельца не решается.

Хвост тоже не обрезан, хотя здесь это было бы дёшево: у jukugo нет знаменателей,
ранги и покрытие он не считает. Но редкое слово — ровно тот случай, когда
инструмент и нужен.
"""
